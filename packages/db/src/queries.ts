...
export interface KnowledgeDetailedRecord {
  campaignId: string;
  sessionId: string;
  title: string | null;
  startedAt: Date;
...
export async function listCurrentDetailedRecordsForKnowledge(): Promise<KnowledgeDetailedRecord[]> {
  const rows = await db
    .select({
      campaignId: sessions.campaignId,
    ...
    .from(sessionArtifacts)
    .innerJoin(sessions, eq(sessionArtifacts.sessionId, sessions.id))
    .where(and(eq(sessionArtifacts.kind, "detailed_record"), eq(sessionArtifacts.isCurrent, true)))
